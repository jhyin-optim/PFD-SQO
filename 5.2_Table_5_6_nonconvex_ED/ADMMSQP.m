...
%%%%%%%%%%计算目标函数值%%%%%%%%%%%%%%%%%%%%%%%%%%%
niter = 1;
obj_f = x_hat_k*qp.q1_UC*x_k +x_k'*qp.C1_UC*x_k+qp.K1_UC'*x_k+sum(qp.d1_UC)+ei*norm(sin(fi*(x_k- qp.x_L)))^2;%f(x)的值
obj_theta = y_hat_k*qp.q2_UC*y_k +y_k'*qp.C2_UC*y_k+qp.K2_UC'*y_k+sum(qp.d2_UC)+ ei*norm(sin(fi*(y_k-qp.y_L)))^2;%theta(y)的值
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%算法循坏%%%%%%%%%%%%%%%%%%%%%%%%%%
 while(niter <= max)
    ...
    xn=size(x_k,1);
    yn=size(y_k,1);
    efc1=sparse(1:xn,1:xn,2*ei*fi*fi*cos(2*fi*(x_k-qp.x_L)));
    efc2=sparse(1:yn,1:yn,2*ei*fi*fi*cos(2*fi*(y_k-qp.y_L)));
    efs1= ei*fi*sin(2*fi*(x_k- qp.x_L));
    efs2= ei*fi*sin(2*fi*(y_k- qp.y_L));
    H_x = H_x_k+betar*(E'*E)+efc1;%x-QP 子问题hesse阵    
    H_y = H_y_k+betar*(F'*F)+efc2;%y-QP 子问题hesse阵 
   f_x=g_x+efs1-H_x_k*x_k+betar*E'*(F*y_k-c-(1/betar)*lamda);%x-QP子问题一次项系数
   f_y=g_y+efs2-H_y_k*y_k+betar*F'*(E*x_k-c-(1/betar)*lamda);%y-QP子问题一次项系数
%    %%%%% solve QP %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%   er2=norm(d_k)/(norm(xyl_k)+1);
   ...
   if er1 <= TOL  || niter>20 %%%%%%%%%%%就是终止准则这里有问题,似乎它永远达不到这个准则，所以有多少次就会执行多少次。（%修改）
       runtime = toc;
    ...
    while (L1==1) 
       % L1=0;
        ...
        f_new = x_hat_new*qp.q1_UC*x_new +x_new'* qp.C1_UC *x_new+qp.K1_UC' *x_new+sum(qp.d1_UC)+ei*norm(sin(fi*(x_new- qp.x_L)))^2;%修改
        theta_new =y_hat_new*qp.q2_UC*y_new +y_new'* qp.C2_UC *y_new+qp.K2_UC' *y_new+sum(qp.d2_UC)+ei*norm(sin(fi*(y_new- qp.y_L)))^2;%修改
       % h_new = 0;
        M_new = E*x_new+F*y_new-c;
        P_new = f_new+theta_new-lamda'*M_new+0.5*betar*norm(M_new)^2;     
